...
class HoloAvatar:
    ...
    def _paint_features(self, p: QPainter, xs, ys, norms, r: float,
        ...
        # ── mouth ───────────────────────────────────────────────────────────
        inner_idx = lm["lips_in"]
        inner_x = xs[inner_idx]
        inner_y = ys[inner_idx]
        # The canonical inner-lip ring is intentionally generous. At HUD
        # scale its filled polygon can read as a black visor instead of a
        # mouth, especially while the head is nearly front-facing. Keep the
        # anatomical outer lip outline, but constrain the cavity itself.
        mouth_cx = float(inner_x.mean())
        mouth_cy = float(inner_y.mean())
        inner = QPolygonF([
            QPointF(
                mouth_cx + (float(x) - mouth_cx) * 0.68,
                mouth_cy + (float(y) - mouth_cy) * 0.56,
            )
