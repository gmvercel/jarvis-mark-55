...
def parse_chord(value: str | tuple[str, ...] | list[str]) -> tuple[str, ...]:
    """Normalize a user-entered chord, falling back to Ctrl+Space."""
    raw = value if isinstance(value, (tuple, list)) else str(value or "").lower().split("+")
    chord = tuple(str(key).strip().lower() for key in raw if str(key).strip())
    return chord if chord and all(key in _VK for key in chord) else DEFAULT_CHORD
