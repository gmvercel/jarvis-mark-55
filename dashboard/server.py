...
from memory.memory_manager import create_work_session, get_work_session, list_work_sessions
...
class DashboardServer:
    ...
    def __init__(self):
        self._ip                          = _local_ip()
        self._port                        = _available_port(PORT)
        ...
        self._active_work_sessions: dict[str, dict] = {}
...
<p>Connecting to JARVIS…</p>
        ...
        async def command(req: Request):
            ...
            if text:
                active_session = self._active_work_sessions.get(token)
                if active_session:
                    context = (
                        f"[Active work session: {active_session.get('title', 'Untitled')}; "
                        f"topic: {active_session.get('topic', 'general')}; "
                        f"briefing: {active_session.get('summary', '')}]\n"
                    )
                    text = context + text
        ...
        async def work_sessions_ep(req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            return JSONResponse({"sessions": list_work_sessions()})

        @app.post("/api/work-sessions")
        async def create_work_session_ep(req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            try:
                body = await req.json()
            ...
            if not title or not topic or not summary:
                return JSONResponse(
                    {"error": "title, topic and summary are required"}, status_code=400
                )
            return JSONResponse({"session": create_work_session(title, topic, summary, scope)})

        @app.get("/api/work-sessions/{session_id}")
        async def get_work_session_ep(session_id: str, req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            session = next(
                (item for item in list_work_sessions() if item.get("id") == session_id),
                None,
            )
            if session is None:
        ...
        @app.post("/api/work-sessions/{session_id}/focus")
        async def focus_work_session_ep(session_id: str, req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            session = next(
                (item for item in list_work_sessions() if item.get("id") == session_id),
            ...
            if session is None:
                return JSONResponse({"error": "Session not found"}, status_code=404)
            token = req.headers.get("authorization", "").removeprefix("Bearer ").strip()
            self._active_work_sessions[token] = session
            return JSONResponse({"ok": True, "session": session})

        @app.post("/api/work-sessions/exit")
        async def exit_work_session_ep(req: Request):
            if not _auth(req):
                return JSONResponse({"error": "Unauthorized"}, status_code=401)
            token = req.headers.get("authorization", "").removeprefix("Bearer ").strip()
            self._active_work_sessions.pop(token, None)
            return JSONResponse({"ok": True})
    ...
    async def _serve_alias(self) -> None:
        ...
        asyncio.get_event_loop().run_in_executor(None, _ensure_network_access, self._port + 1)
        cfg = uvicorn.Config(
            self.app, host="0.0.0.0", port=self._port + 1, log_level="warning",
            ssl_keyfile=str(ssl_key), ssl_certfile=str(ssl_cert),
        )
        print(f"[Dashboard] Manual entry:  {self._ip}:{self._port + 1}  (type in browser, accept cert once)")
