...
GEMINI_FALLBACK_MODELS = ("gemini-flash-lite-latest", "models/gemini-3.6-flash")
...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    if output_path:
        ...
        if parts and parts[0] in {"desktop", "desktop".lower()}:
            return DESKTOP / Path(*p.parts[1:])
        return DESKTOP / p
...
def _resolve_input_path(file_path: str) -> Path:
    """Resolve relative coding paths from the project root, not the shell cwd."""
...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    file_exists = bool(file_path) and _resolve_input_path(file_path).exists()
...
Code:"""
    ...
    try:
        ...
        if not any(marker in error_text for marker in ("504", "deadline", "timeout", "timed out")):
            raise

        short_prompt = (
            f"Write a complete runnable {language} program. Output ONLY raw code, no markdown.\n"
        ...
        for fallback_model in GEMINI_FALLBACK_MODELS:
            try:
                print(f"[Code] Retrying with {fallback_model}...")
                response = _get_gemini(fallback_model).generate_content(short_prompt)
                break
            except Exception as fallback_error:
                fallback_errors.append(f"{fallback_model}: {fallback_error}")
        if response is None:
            raise RuntimeError("; ".join(fallback_errors)) from first_error
    code     = _clean_code(response.text)
    if not code:
        raise RuntimeError("Gemini returned empty code")
    path     = _resolve_save_path(output_path, lang)
    save_status = _save_file(path, code)
    if save_status.startswith("Could not save"):
...
def _write_action(description, language, output_path, player) -> str:
    ...
    try:
        ...
        error_text = str(e)
        if "404" in error_text and "NOT_FOUND" in error_text:
            return (
                "Could not generate code: the configured Gemini model is unavailable "
                "for this API key. Check the Gemini API key and enabled models."
            )
        if any(marker in error_text.lower() for marker in ("504", "deadline", "timeout", "timed out")):
            return "Could not generate code: Gemini timed out twice. Check the connection or try a shorter request."
        return f"Could not generate code: {error_text}"
...
Updated code:"""
    ...
    status = _save_file(_resolve_input_path(file_path), edited)
...
def _run_action(file_path, args, timeout, player) -> str:
    ...
    return _run_file(p, args or [], timeout)
...
Optimized code:"""
    ...
    # Kaydet
    if file_path:
        save_path = _resolve_input_path(file_path)
    else:
