...
def close_app(
    ...
    """Close running processes matching an application alias or executable."""
    app_name = (parameters or {}).get("app_name", "").strip()
    if not app_name:
        return "No application name provided."
    if not _PSUTIL:
    ...
    target_names = {
        ...
        for name in (app_name, normalized)
        if name
    }
    closed = 0
    ...
    try:
        for process in psutil.process_iter(["pid", "name", "exe"]):
            if process.info["pid"] == current_pid:
                continue
            process_name = Path(process.info.get("name") or "").stem.casefold()
            executable = Path(process.info.get("exe") or "").stem.casefold()
            if not ({process_name, executable} & target_names):
                continue
            try:
                process.terminate()
                closed += 1
        ...
        if closed:
            _, still_running = psutil.wait_procs(
            ...
            for process in still_running:
                try:
                    if Path(process.info.get("name") or "").stem.casefold() in target_names:
                        process.kill()
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    pass
