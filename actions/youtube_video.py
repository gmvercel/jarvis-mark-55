...
def _handle_play(parameters: dict, player) -> str:
    ...
    if video_url:
        ...
        _open_url(video_url)
        if player and hasattr(player, "set_music_track"):
            video_id = _extract_video_id(video_url)
            cover_path = _cache_thumbnail(video_id) if video_id else None
            title, artist = _video_metadata(video_url, query)
            player.set_music_track(title, artist, cover_path)
    ...
    if player and hasattr(player, "set_music_track"):
        player.set_music_track(query, "YouTube", None)
    return f"Opened YouTube search for: {query} (manual selection required)"


def _cache_thumbnail(video_id: str) -> str | None:
    """Cache a YouTube thumbnail locally for the in-app now-playing card."""
    if not _REQUESTS_OK or not video_id:
        return None
    cache_dir = BASE_DIR / "cache" / "music"
    try:
        cache_dir.mkdir(parents=True, exist_ok=True)
        target = cache_dir / f"{video_id}.jpg"
        ...
        print(f"[YouTube] Thumbnail unavailable: {exc}")
        return None


def _video_metadata(video_url: str, fallback_title: str) -> tuple[str, str]:
    if not _REQUESTS_OK:
    ...
    try:
        ...
        response.raise_for_status()
        data = response.json()
        return data.get("title") or fallback_title, data.get("author_name") or "YouTube"
    except Exception as exc:
        print(f"[YouTube] Metadata unavailable: {exc}")
        return fallback_title, "YouTube"
