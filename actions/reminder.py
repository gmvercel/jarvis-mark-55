...
def _write_notify_script(task_name: str, message: str, os_name: str) -> Path:
    ...
    event_dir_literal = json.dumps(str(_events_dir()))
...
try:
    import json
    from pathlib import Path
    event_dir = Path({event_dir_literal})
    event_dir.mkdir(parents=True, exist_ok=True)
    event_path = event_dir / (Path(__file__).stem + ".json")
    event_data = {{"message": message, "created_at": time.strftime("%Y-%m-%d %H:%M:%S")}}
    event_path.write_text(json.dumps(event_data), encoding="utf-8")
    history_path = event_dir / "history.json"
    history = json.loads(history_path.read_text(encoding="utf-8")) if history_path.exists() else []
    history.append(event_data)
    history_path.write_text(json.dumps(history[-100:]), encoding="utf-8")
except Exception:
    pass
"""
