...
def _unique_path(target: Path, kind: str = "file") -> Path:
    """Create a conflict-free target path without silently overwriting."""
    if not target.exists():
        return target
...
def create_file(path: str, name: str = "", content: str = "") -> str:
    try:
        if not name:
            return "Name is required to create a new file."
        ...
        if target.exists() and target.is_dir():
            return f"Cannot create file: '{target}' is a directory."

        if target.exists():
            target = _unique_path(target, kind="file")
...
def create_folder(path: str, name: str = "") -> str:
    try:
        ...
        if target.exists() and target.is_file():
            return f"Cannot create folder: '{target}' is a file."
        if target.exists():
            target = _unique_path(target, kind="folder")
...
def write_code_to_file(path: str, name: str = "", content: str = "",
                        append: bool = False, language: str = "python") -> str:
    """Write or append code into a developer file using the controller's safe file path rules.

    This is an explicitly requested capability: the assistant can write code snippets or
    research/writeable material into files inside the user's own safe workspace.
    """
    try:
        if not name:
            return "Name is required to write a code file."
        ...
        action = "Appended code to" if append else "Wrote code to"
        return f"{action}: {target.name} ({language})"
    except Exception as e:
        return f"Could not write code file: {e}"


def create_project_structure(path: str, name: str = "", structure: dict = None,
                               create_readme: bool = True,
                               language: str = "python") -> str:
    """Create a multi-file project scaffold from a simple nested structure map.

    Example structure:
        {"src": {"main.py": "print('hello')"}, "tests": {"test_main.py": ""}}
    """
    try:
        if not name:
            name = "project"
        ...
        base = _resolve_path(path)
        target_dir = (base / name) if name else base
        if not _is_safe_path(target_dir):
            return f"Access denied: {target_dir}"

        if target_dir.exists() and target_dir.is_file():
            return f"Cannot create project structure: '{target_dir}' is a file."

        if target_dir.exists():
            # avoid clobbering existing folders silently
            target_dir = _unique_path(target_dir, kind="folder")

        target_dir.mkdir(parents=True, exist_ok=True)

        structure = structure or {}

        for item, payload in structure.items():
            item_path = target_dir / item
            if isinstance(payload, dict):
                item_path.mkdir(parents=True, exist_ok=True)
                # recursively build nested structure from mapping
                for child, child_payload in payload.items():
                    child_path = item_path / child
                    if isinstance(child_payload, dict):
                        child_path.mkdir(parents=True, exist_ok=True)
                        for grandchild, gc_payload in child_payload.items():
                            grandchild_path = child_path / grandchild
                            if isinstance(gc_payload, dict):
                                grandchild_path.mkdir(parents=True, exist_ok=True)
                            else:
                                grandchild_path.parent.mkdir(parents=True, exist_ok=True)
                                grandchild_path.write_text(str(gc_payload), encoding="utf-8")
                    else:
                        child_path.parent.mkdir(parents=True, exist_ok=True)
                        child_path.write_text(str(child_payload), encoding="utf-8")
            else:
                item_path.parent.mkdir(parents=True, exist_ok=True)
                item_path.write_text(str(payload), encoding="utf-8")

        if create_readme:
            readme_path = target_dir / "README.md"
            readme_path.write_text(
                f"# {target_dir.name}\n\nGenerated project scaffold for {language} language.\n",
                encoding="utf-8",
            )

        return f"Project structure created: {target_dir.name}"
    except Exception as e:
        return f"Could not create project structure: {e}"
...
def file_controller(
    ...
    try:
        if action == "open":
            target = Path(path)
            if not target.is_absolute() and name:
                target = _resolve_path(path) / name
            elif not target.is_absolute() and not name:
                target = _resolve_path(path)
            if not target.exists() and name:
                target = _resolve_path(path) / name
            if not target.exists():
                return f"File or folder not found: {target}"
            if player and hasattr(player, "show_file_preview"):
                player.show_file_preview(str(target))
            kind = "folder" if target.is_dir() else "file"
            return f"Opening {kind} preview for {target.name} in the holographic interface."
        ...
        if action == "list":
            ...
            return create_project_structure(
                path,
                name=name or params.get("project_name", "project"),
                structure=params.get("structure", None),
                create_readme=params.get("create_readme", True),
                language=params.get("language", "python"),
            )
