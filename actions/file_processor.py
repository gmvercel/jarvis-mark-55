...
def _normalize_file_path_from_parameters(parameters: dict) -> str:
    """Return a usable file path from any common alias used by the assistant UI,
    upload endpoints, and tool invocations.

    Supports the canonical key `file_path` and the most frequent alternatives:
    `path`, `file`, `filename`, `filepath`, `uploaded_file`, `uploaded_path`,
    and nested ``file`` dicts.
    """
    if not isinstance(parameters, dict):
        return ""

    keys = (
        "file_path", "filePath", "filepath", "path", "file",
        "filename", "file_name", "uploaded_file", "uploaded_path",
        "source_file", "document_path", "image_path",
    )
    ...
    def _candidate(raw):
        ...
        if isinstance(raw, Path):
            return str(raw.expanduser())
        if isinstance(raw, dict):
            for nested_key in ("path", "file_path", "filepath", "filename", "file_name", "name"):
    ...
    for key in keys:
        if key in parameters:
            value = _candidate(parameters[key])
            if value:
                ...
                if value:
                    ...
                    if p.exists() and p.is_file():
                        return str(p)
                # try to recover from an internal upload naming alias that is just a filename
                if Path(value).name == value:
                    p = Path.cwd() / "uploads" / value
                    if p.exists() and p.is_file():
                        return str(p)

    # final fallback: inspect "current_file" off the player if present
    player = parameters.get("player") if isinstance(parameters, dict) else None
    if player is None:
        # no-op: consumer can pass object later via top-level explicit code path
        pass
    return ""
...
def file_processor(parameters: dict, player=None, speak=None) -> str:
    # Rebuild the file selection from scratch so it works in the same way
    # regardless of whether the caller sends file_path, path, file, file_name,
    # uploaded_file or another alias from the chat/upload surface.
    file_path_str = _normalize_file_path_from_parameters(parameters)
    if not file_path_str:
        if player and getattr(player, "current_file", None):
            file_path_str = str(player.current_file)

    if not file_path_str:
        # Backwards-compatible final fallback for the built-in declaration
