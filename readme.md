...
| ◈ Work Sessions | Create, review, annotate, and activate persistent project sessions from the UI |
...
| 🎵 Spotify Control | Play tracks and playlists, pause, resume, and skip tracks through Spotify |
...
| **Spotify** | Spotify Premium account and a Spotify Developer app for direct playback |

### Spotify setup

1. Create an application at the [Spotify Developer Dashboard](https://developer.spotify.com/dashboard) and set its redirect URI to `http://127.0.0.1:8888/callback`.
2. Add these fields to `config/api_keys.json` (or use the equivalent environment variables `SPOTIFY_CLIENT_ID`, `SPOTIFY_CLIENT_SECRET`, and `SPOTIFY_REDIRECT_URI`):
...
The first Spotify command opens the authorization page. The token is cached in the user profile, outside the repository. Spotify must be open on at least one device and direct playback requires Spotify Premium. Voice commands include "riproduci ...", "metti in pausa Spotify", "brano successivo", and "brano precedente".
...
│   └── spotify.py            # Spotify Web API playback control
├── actions/
